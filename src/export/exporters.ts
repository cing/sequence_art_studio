...
export async function exportPdf(
  ...
  const mode = clonedSvg.getAttribute('data-art-mode');
  ...
  // Work around svg2pdf rendering gaps with Wang tile polygons in some viewers.
  if (mode === 'wang_maze') {
    const viewBox = clonedSvg.getAttribute('viewBox') ?? '';
    const [, , widthText, heightText] = viewBox.trim().split(/\s+/);
    const width = Math.max(1, Math.round(Number(widthText) || 1800));
    const height = Math.max(1, Math.round(Number(heightText) || 1800));
    ...
    try {
      const image = await new Promise<HTMLImageElement>((resolve, reject) => {
        const img = new Image();
        img.onload = () => resolve(img);
        img.onerror = () => reject(new Error('Unable to render SVG for Wang PDF export.'));
        img.src = url;
      });
      ...
      const context = canvas.getContext('2d');
      if (!context) {
        throw new Error('Canvas context not available for Wang PDF export.');
      }
      context.drawImage(image, 0, 0, width, height);

      const pngDataUrl = canvas.toDataURL('image/png');
      pdf.addImage(pngDataUrl, 'PNG', 0, 0, widthIn, heightIn, undefined, 'FAST');
      pdf.save(filename);
      return;
    } finally {
      URL.revokeObjectURL(url);
